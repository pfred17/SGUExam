...
namespace GUI.modules
{
    public partial class UC_ThemNhomHocPhan : UserControl
    {
        ...
        {
            string query = "SELECT DISTINCT hoc_ky FROM nhom_hoc_phan";
            DataTable dt = DAL.DatabaseHelper.ExecuteQuery(query);

            cbHocKy.DataSource = dt;
            cbHocKy.DisplayMember = "hoc_ky";
            cbHocKy.ValueMember = "hoc_ky";
        }
        // load combo box năm học
        private void LoadNamHoc()
        {
            string query = "SELECT DISTINCT nam_hoc FROM nhom_hoc_phan";
            DataTable dt = DAL.DatabaseHelper.ExecuteQuery(query);
        ...
        public void ResetFields()
        {
            //tbTenNhom.Text = "";
            //tbGhiChu.Text = "";           
            //// reset combobox về -1 hoặc default
